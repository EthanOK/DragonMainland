...
contract DragonMainlandTokenV2 is
    ERC721,
    ERC721URIStorage,
    ERC721Burnable,
    ReentrancyGuard,
    DragonBase
{
    ...
    function createDragonEggs(
        ...
        if (_matronId > 0) {
            ...
            // add  breed count
            dragonBreedCount[_matronId]++;
            emit BreedCount(_owner, _matronId, dragonBreedCount[_matronId]);
        }
        if (_sireId > 0) {
            dragonChildrens[_sireId].push(_tokenId);
            // add  breed count
            dragonBreedCount[_sireId]++;
            emit BreedCount(_owner, _sireId, dragonBreedCount[_sireId]);
        }
    ...
    function _setDragonMeta(
        ...
        emit DragonMetaLog(_tokenId, _geneDomi, _geneRece, _stage);
    }
        ...
        returns (bool)
    {
        ...
        // breed count
        require(
            dragonBreedCount[_matronId] < breedCountMax,
            "matron breed count max"
        );
        require(
            dragonBreedCount[_sireId] < breedCountMax,
        ...
        // create dragon egg
        require(
            createDragonEggs(_tokenId, _job, _matronId, _sireId, _owner),
        ...
        cooldownTimeEnd[_matronId] =
            block.timestamp +
            breedCooldown[dragonBreedCount[_matronId] + 1];
        cooldownTimeEnd[_sireId] =
            block.timestamp +
            breedCooldown[dragonBreedCount[_sireId] + 1];

        emit DragonBreedLog(_tokenId, _job, _matronId, _sireId, _owner);
        return true;
    }
}
