    ...
    ReentrancyGuard
{
    ...
    // pay type
    event PayType(uint256 newPay, uint256 oldPay);
    ...
    // pay type 1=dms 2=bnb
    uint256 public payType = 1;

    // exchange data
    struct ExchangeData {
        address from;
        address to;
        uint256 pay_type; // 1=dms 2=bnb
        uint256 price; // price
        uint256 create;
    ...
    // set pay type dms & bnb
    function setPayType(uint256 _pay) external onlyOwner {
        require(_pay == 1 || _pay == 2, "invalid pay type");
        emit PayType(_pay, payType);
        payType = _pay;
    }
    ...
    function addMarket(
        ...
        require(payType == 1, "invalid payType");
    ...
    function addMarketV2(
        ...
        require(payType == 2, "invalid payType");
        require(!exchangeDatas[_tokenId].state, "nft is in market");
        require(
            dragonToken.ownerOf(_tokenId) == msg.sender,
            "invalid nft owner"
    ...
    // bnb token transfer earn
    function bnbTransferEarn(address _from, uint256 _amount) private {
        uint256 bnbBalance = _from.balance;
        require(bnbBalance >= _amount, "BNB balance is not enough");
        payable(beneficiary).transfer(_amount);
    }

    // bnb token transferFrom
    function bnbTransferFrom(
        address _from,
        address _to,
        uint256 _amount
    ) private {
    ...
    // nft exchange
    // transfer bnb token
    function exchangeV2(uint256 _tokenId, uint256 _orderId)
        external
        payable
        whenNotPaused
        nonReentrant
        ...
        returns (bool)
    {
        ...
        uint256 _fee = exchangeFee(_price);
        // bnb token
        bnbTransferEarn(msg.sender, _fee);
        bnbTransferFrom(msg.sender, _exdata.from, _price - _fee);
        // dragon nft
        dragonToken.safeTransferFrom(address(this), msg.sender, _tokenId);
        _exdata.to = msg.sender;
        _exdata.state = false;
