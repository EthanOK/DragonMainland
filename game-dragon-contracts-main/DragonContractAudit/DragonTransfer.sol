...
contract DragonTransfer is Ownable {
    ...
    event BreedDmsAmt(uint256[7] amounts);
    // Breed DMP amounts event
    event BreedDmpAmt(uint256[7] amounts);
    ...
    IERC1155 public dmbToken =
        IERC1155(0x482c27c754454BC184C5bCDbf90272628f220748);
    ...
    function setBreedDmsAmt(uint256[7] calldata _amounts) external onlyOwner {
        for (uint256 i = 1; i < _amounts.length; i++) {
            require(_amounts[i - 1] > 0, "amount is zero");
            breedDmsAmt[i] = _amounts[i - 1];
        }
        emit BreedDmsAmt(_amounts);
    }

    // breed dragon DMP token amount
    function setBreedDmpAmt(uint256[7] calldata _amounts) external onlyOwner {
        for (uint256 i = 1; i < _amounts.length; i++) {
            require(_amounts[i - 1] > 0, "amount is zero");
            breedDmpAmt[i] = _amounts[i - 1];
        }
